<?php
	if ($_POST['sms_send_'.$sms_section_from]){
		...
		if ($_POST[$_POST['UniqueFieldID']] == $getUniqueSMSID):
			...
			$SMSUsername			= get_option('sms_username');
			$SMSPassword			= get_option('sms_password');
			$sms_send_response  		= wp_remote_fopen('https://www.suresms.com/scripts/GlobalSendSMS.aspx?login='.$SMSUsername.'&password='.$SMSPassword.'&to='.$toSendSMSNumber.'&text='.$toSendSMSMessage.'&footer_text='.$toSendSMSFooterText);
...
<div id="send_sms">
    ...
    <ul class="sms_form">
        ...
        <li>
        	<label>Message</label>
            <textarea class="required" name="sms_mob_msg" maxlength="500"></textarea>
